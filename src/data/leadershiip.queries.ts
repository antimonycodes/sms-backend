...
JOIN sessions ses ON ses.id = sl.session_id
JOIN terms t ON t.id = sl.term_id
WHERE sl.school_id = $1
  AND sl.session_id = COALESCE($2, (SELECT id FROM sessions WHERE school_id = $1 AND is_active = true LIMIT 1))
